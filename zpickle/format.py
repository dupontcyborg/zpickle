...
from .exceptions import InvalidFormatError, UnsupportedAlgorithmError, UnsupportedVersionError

# Format constants
ZPICKLE_MAGIC = b'ZPKL'
PROTOCOL_VERSION = 1           # Format version
HEADER_FMT = '!4sBBBB'        # Magic (4), Version (1), Algorithm ID (1), Level (1), Reserved (1)
...
def encode_header(algorithm: str, level: int, reserved: int = RESERVED_BYTE) -> bytes:
    ...
    return struct.pack(HEADER_FMT, ZPICKLE_MAGIC, PROTOCOL_VERSION, alg_id, level, reserved)
...
def decode_header(data: bytes, strict: bool = True) -> Tuple[int, str, int, int]:
    ...
    if version > PROTOCOL_VERSION:
        msg = (f"File uses zpickle protocol version {version}, but this code only "
    ...
    if version == 1 and reserved != 0 and strict:
        warnings.warn(f"Reserved byte is {reserved}, expected 0 for version 1.", RuntimeWarning)
